...
class ShortLink(Base):
    ...
    short: Mapped[str] = mapped_column(String(12), unique=True)
    original: Mapped[str] = mapped_column(String)
    alias: Mapped[Optional[str]] = mapped_column(String, unique=True, nullable=True)
    redirects_count: Mapped[int] = mapped_column(default=0)
    user_id: Mapped[int] = mapped_column(ForeignKey("users.id"))

    @validates("kind")
    def validate_kind(self, key, kind):
        if kind not in ["public", "protected", "private"]:
    ...
    def to_dict(self) -> dict:
        return {'id': self.id, 'kind': self.kind, 'short': self.short, 'alias': self.alias,
                'redirects_count': self.redirects_count, 'user_id': self.user_id}
