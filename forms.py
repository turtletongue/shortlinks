...
class RegistrationForm(FlaskForm):
  ...
  password_again = PasswordField("Подтвердите пароль", validators=[DataRequired(), Length(int(os.environ['MIN_PASSWORD_LENGTH']))])

  submit = SubmitField('Зарегистрироваться')

class CreateLinkForm(FlaskForm):
  link = StringField("Ссылка", validators=[DataRequired(), Length(3)])
  type = SelectField("Тип", choices=[('public', 'Публичная'), ('protected', 'Общего доступа'), ('private', 'Приватная')], validators=[DataRequired()])
